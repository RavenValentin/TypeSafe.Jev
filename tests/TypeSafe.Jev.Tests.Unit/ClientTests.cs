...
public class ClientTests
{
    ...
    public async Task A_span_is_produced_with_operation_model_and_status()
    {
        ...
        var activity = Assert.Single(activities, a => (string?)a.GetTagItem("typesafe_jev.request.model") == probe);
        Assert.Equal("jev system_one", activity.OperationName);
        Assert.Equal("system_one", activity.GetTagItem("typesafe_jev.operation"));
        Assert.Equal(200, activity.GetTagItem("http.response.status_code"));
        Assert.Equal("jev-1.13.0", activity.GetTagItem("typesafe_jev.response.model"));
