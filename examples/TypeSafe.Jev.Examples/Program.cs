...
using TypeSafe.Jev;
using TypeSafe.Jev.Examples;

// Probabilities read the same on every machine.
CultureInfo.DefaultThreadCurrentCulture = CultureInfo.InvariantCulture;

// Run one:      dotnet run --project examples/TypeSafe.Jev.Examples -- 03
// Run all:      dotnet run --project examples/TypeSafe.Jev.Examples -- all
...
if (selection is null)
{
    ...
    Console.WriteLine("\nUsage: dotnet run --project examples/TypeSafe.Jev.Examples -- <id|all> [--offline]");
