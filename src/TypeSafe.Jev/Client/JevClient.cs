...
public sealed class JevClient : IJevClient, IDisposable
{
    ...
    public JevClient(HttpClient httpClient, JevClientOptions options)
    {
        ...
        _log = new Log((options.LoggerFactory ?? NullLoggerFactory.Instance).CreateLogger("TypeSafe.Jev"));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 0; ; attempt++)
        {
            ...
            JevTelemetry.Retries.Add(1, new KeyValuePair<string, object?>("typesafe_jev.operation", operation));
    ...
    private Activity? StartActivity(string operation, string? model)
    {
        ...
        activity.SetTag("typesafe_jev.operation", operation);
        ...
        if (model is not null) activity.SetTag("typesafe_jev.request.model", model);
        return activity;
    }
    ...
    private void Complete(Activity? activity, string operation, int? status, int attempt, long started, string? error)
    {
        ...
        {
            { "typesafe_jev.operation", operation },
    ...
    private static void Record(Activity? activity, SystemOneResponse result)
    {
        if (result.Usage?.InputTokens is { } input)
            JevTelemetry.Tokens.Add(input, new KeyValuePair<string, object?>("typesafe_jev.token.type", "input"),
                new KeyValuePair<string, object?>("typesafe_jev.response.model", result.Model));
        if (result.Usage?.OutputTokens is { } output)
            JevTelemetry.Tokens.Add(output, new KeyValuePair<string, object?>("typesafe_jev.token.type", "output"),
                new KeyValuePair<string, object?>("typesafe_jev.response.model", result.Model));

        activity?.SetTag("typesafe_jev.response.model", result.Model);
        activity?.SetTag("typesafe_jev.usage.input_tokens", result.Usage?.InputTokens);
        activity?.SetTag("typesafe_jev.request_id", result.RequestId);
    }
